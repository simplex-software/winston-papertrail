...
var Papertrail = exports.Papertrail = function (options) {
    ...
    self.breakMultilineMessages = options.breakMultilineMessages || false;
 ...
 * @returns {*}
 */
Papertrail.prototype.log = function (defaultLevel, defaultMsg, args, callback) {
    const getArgsValue = (key) => Object.getOwnPropertySymbols(args).filter(k => util.inspect(k).includes(key)).map(k => args[k])[0]

    var self = this;
    level = args ? getArgsValue('level') : defaultLevel || defaultLevel
    msg = args ? getArgsValue('message') : defaultMsg || defaultMsg,
    meta = args ? getArgsValue('splat') : null;
    ...
    var output = JSON.stringify({ ...JSON.parse(msg), meta });
...
Papertrail.prototype.sendMessage = function (hostname, program, level, message, callback) {
    ...
    if (message) {
        if (self.breakMultilineMessages){
            lines = message.split('\n');
        }else{
            lines = [message];
        }
